...
def dashboard_by_id(request, id):
	...
	if older_than is None or older_than == 0 or older_than == '':
		older_than = now().timestamp()
	older_than = __timestamp_to_utc(older_than)
...
def register(request):
	...
	try:
		...
		new_password_confirm = body.get('password_confirm', None)
